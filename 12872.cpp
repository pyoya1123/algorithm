...
그리고 j - M 을 곱해줬는데, a b c d e f 가 있다고 치자.
이때 M 값이 4라고 가정하면, a x x x x a 가 성립하려면 중간에 서로 다른 문자가 4개가 있어야한다는거임. 즉, 마지막 a 자리에 올 수 있는 문자는
6개 중에 4개를 뺀 2개가 올 수 있음. 그래서 j - M을 곱해준거임.
